...
execute unless data storage bs:const log.messages[{namespaces:["bs.interaction"]}] run \
  data modify storage bs:const log.messages[{namespaces:["bs"]}].namespaces append value "bs.interaction"
