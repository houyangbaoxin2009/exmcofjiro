...
data modify storage bs:data load.modules[{module:"bs.view",version:"4.0.1"}].enabled set value 1b
...
execute unless score #bs.view.major_versions load.status matches 2.. \
  if score $bs.view.major load.status matches 4 \
  if score $bs.view.minor load.status matches 0 \
  if score $bs.view.patch load.status matches 1 \
  run return 1
