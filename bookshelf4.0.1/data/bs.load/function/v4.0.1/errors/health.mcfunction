...
execute unless score $bs.health.minor load.status matches 0 run return 0
execute unless score $bs.health.patch load.status matches 1 run return 0

# Format an error message for conflicting versions
data modify storage bs:data load.errors append value {major:1b,message:{text:"[bs.health]",hover_event:{action:"show_text",value:[{text:"[v",color:"#CCCCCC"},{type:"nbt",storage:"bs:data",nbt:"load.modules[{module:'bs.health',enabled:1b}].version"},{text:" ("},{text:"✔ enabled",color:"#26BD26"},{text:"), v"},{type:"nbt",storage:"bs:data",nbt:"load.modules[{module:'bs.health',enabled:0b}].version","separator":", v"},{text:"]"}]}}}
...
function bs.load:v4.0.1/bundle/append {module:"bs.health"}
