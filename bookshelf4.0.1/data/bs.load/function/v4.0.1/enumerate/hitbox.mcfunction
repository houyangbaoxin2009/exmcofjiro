...
data modify storage bs:data load.modules append value {module:"bs.hitbox",version:"4.0.1",enabled:0b}
...
execute unless score $bs.hitbox.major load.status matches 4 run scoreboard players add #bs.hitbox.major_versions load.status 1
...
execute if score $bs.hitbox.major load.status matches 4 if score $bs.hitbox.minor load.status matches 0 unless score $bs.hitbox.patch load.status matches ..1 run return 0
