...
execute unless data storage bs:const log.messages[{namespaces:["bs.health"]}] run \
  data modify storage bs:const log.messages[{namespaces:["bs"]}].namespaces append value "bs.health"
